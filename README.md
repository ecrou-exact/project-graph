...
- **Hugo**: a component that lets a Hugo site build the graph's JSON from its own pages (front matter `graph` and `relations`) or from a data file. Without JavaScript, Hugo draws the graph as SVG and writes it out as text; with JavaScript, the interactive graph (filters, search, zoom) takes over. See [Hugo](https://ecrou-exact.github.io/project-graph/docs/guide.html#hugo) and the [live example](https://ecrou-exact.github.io/project-graph/hugo-example/projects/).
